...
function itm_posted_on() {
	...
	// Translators: %s: post date.
		esc_html_x( 'Posted on %s', 'post date', 'it_book' ),
		$time_string
	);
...
function itm_posted_by() {
	$byline = sprintf( /* translators: %s: post author. */ esc_html_x( 'by %s', 'post author', 'it_book' ), '<span class="author vcard"><a class="url fn n" href="' . esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ) . '">' . esc_html( get_the_author() ) . '</a></span>' );
...
function itm_cat_links() {
	$categories_list = get_the_category_list( esc_html__( ', ', 'it_book' ) );
	if ( $categories_list ) {
		// Translators: category list.
		printf( '<div class="cat-links">' . esc_html__( 'Posted in %1$s', 'it_book' ) . '</div>', $categories_list ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
	}
}
...
function itm_tag_links() {
	...
	if ( $tags_list ) {
		...
		printf( '<div class="tag-links">' . esc_html__( 'Tagged %1$s', 'it_book' ) . '</div>', $tags_list ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
	}
}
...
/**
 * Append primary term from Book by ID
 */
function itm_term_cat_book_by_id( $get_id ) {
	$wpseo_primary_term = new WPSEO_Primary_Term( 'book-category', $get_id );
	$wpseo_primary_term = $wpseo_primary_term->get_primary_term();
	$term_cat = get_term( $wpseo_primary_term );

	if ( $wpseo_primary_term ) {
		echo '<a class="cat-book c-a-underline" href="' . esc_url( get_term_link( $term_cat->term_id ) ) . '">' . wp_kses_post( $term_cat->name ) . '</a>';
	}
}